		...
		function cleanValidationMsgs(){
			...
			$('#divMsgAlMenosUnParticipante').hide();
		...
		$(window).bind("pageshow", function() {
			var loadedCount = $("#hdnParticipantesWidgetLoadedCount").val();
			if(loadedCount == ""){
				$("#hdnParticipantesWidgetCount").val(0);
			}
			else{
				$("#hdnParticipantesWidgetCount").val(loadedCount);
			}
		});
		...
		$('#frmApp').submit( function(event){	
			...
			if( parseInt($('#hdnParticipantesWidgetCount').val()) <= 0 ){
				errorMsg.push({ errName: 'Datos requeridos', errField: 'Participante' });
				$('#divMsgAlMenosUnParticipante').show();
				valid = false;
			}
